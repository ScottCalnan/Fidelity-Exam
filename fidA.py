...
# of C in the first third of S are then replaced by instances of C_shift in the remaing two thirds of S. Unit code of all the fnctions created is at the bottom.
...
def switch_instance(S,C,N):
	...
	# search for C in 1st third of S and for C_shift in 2nd and 3rd thirds of S
	if length(find_sub_string(first_third_S, C)) < length(find_sub_string(rest_of_S, C_shift)):
		iterate = length(find_sub_string(first_third_S, C)) # less instances of C
	else:
		iterate = length(find_sub_string(rest_of_S, C_shift)) # less instances of C_shift
		...
		if (find_sub_string(first_third_S, C)[i] + length(C) < length(first_third_S)):
			if find_sub_string(first_third_S, C) != -1:
				...
				if find_sub_string(rest_of_S, C_shift) != -1:
					i2 = (find_sub_string(rest_of_S, C_shift)[i] + length(first_third_S)) # get index of c_shift
